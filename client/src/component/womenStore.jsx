...
const SearchHandler = (e) => {
  ...
  navigate(`/store/Women?s=${encodedSearchInput}`);
  axios.get(`/posts/women?s=${searchInput}`)
    .then((res) => {
...
useEffect(()=>{
  ...
  setisloading(true)
axios.get(`/posts/women?s=${searchQuery || ""}`)
.then((res)=>{
  console.log(res.data)
  setProducts(res.data)
      ...
      <div className='bg-[#f5f7f9] max-[600px]:flex-col px-[30px] flex justify-around md:py-[70px] py-[50px]'>
        ...
        <div className='pt-[100px] bg-white max-w-[1000px] w-[95%] px-[5%] flex flex-col items-center ' >
         <div className=' px-[13px] w-full flex  justify-start pb-[2em]'>
          <h1 className='text-[#777] '>{searchQuery || searchInput ? (`Home/Store/Women ${searchQuery}`) :'Home/Store/Women'}</h1>
         </div>
         <div>
            <h1 className='text-[66px] max-[450px]:text-3xl mb-[1em]'>Women</h1>
